...
public class cameraController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetAxisRaw("Vertical") < 0) transform.position = new Vector2(transform.position.x, transform.position.y-speed*Time.deltaTime);
        if (Input.GetKey(KeyCode.Q) && GetComponent<Camera>().orthographicSize>1) GetComponent<Camera>().orthographicSize--;
        if (Input.GetKey(KeyCode.E)) GetComponent<Camera>().orthographicSize++;
